function getStreams(tmdbId, mediaType, seasonNum, episodeNum) {
    ...
    return fetch(`${base}/v1/streams`, {
        ...
        if (!response.ok) {
            throw new Error(payload.error || `MovieBox bridge HTTP ${response.status}`);
        }
        const streams = Array.isArray(payload.streams) ? payload.streams : [];
        console.log(
            `[MovieBoxBridge] ${streams.length} stream(s) for ${body.mediaType}/${body.tmdbId}` +
            (body.mediaType === 'tv' ? ` S${body.season}E${body.episode}` : '')
        );
        return streams;
    }).catch(error => {
...
async function getMovieBoxStreams(tmdbId, mediaType = 'movie', seasonNum = null, episodeNum = null) {
    return getStreams(tmdbId, mediaType, seasonNum, episodeNum);
}
