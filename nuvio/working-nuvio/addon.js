...
const { getRepoProviders } = require('./repo-loader.js');
const { wrapWithMediaFlow } = require('./mediaflow-wrap.js');
...
builder.defineStreamHandler(async (args) => {
    ...
    // -------------------------------------------------
    // Dynamic Nuvio repository providers
    // -------------------------------------------------
    const repoProviders = await getRepoProviders();
    const repoProviderMeta = [];

    for (const rp of repoProviders) {
        const key =
            `repo_${rp.repoId}_${rp.id}`
                .toLowerCase()
                .replace(/[^a-z0-9_]/g, '_');
        ...
        providerFetchFunctions[key] = async () => {
            const supported =
                Array.isArray(rp.supportedTypes)
                    ? rp.supportedTypes.map(
            ...
            if (
                !supported.includes(
                    String(tmdbTypeFromId).toLowerCase()
                )
            ) {
                return [];
            }
            ...
            try {
                console.log(
                    `[RepoProvider] ${displayName} fetching...`
                );
                ...
                console.log(
                    `[RepoProvider] ${displayName}: ${streams.length} stream(s)`
                );
    ...
    try {
        ...
        const repoProviderStartIndex =
            providerPromises.length;

        const repoProviderNames =
            repoProviderMeta.map(
                meta => meta.displayName
            );

        for (const meta of repoProviderMeta) {
            providerPromises.push(
                timeProvider(
                    meta.displayName,
                    providerFetchFunctions[meta.key]()
                )
            );
        }
        ...
        // Add dynamically loaded repository providers
        repoProviderMeta.forEach((meta, i) => {
            const resultIndex =
                repoProviderStartIndex + i;

            const raw =
                providerResults[resultIndex] || [];

            streamsByProvider[meta.displayName] =
                applyAllStreamFilters(
                    raw,
                    meta.displayName,
                    minQualitiesPreferences[meta.id],
                    excludeCodecsPreferences[meta.id]
                );
        });
    ...
    const stremioStreamObjects = combinedRawStreams.map((stream) => {
        stream = wrapWithMediaFlow(stream);
